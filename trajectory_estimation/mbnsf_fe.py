# Long-term trajectory estimation with MBNSF and forward-Euler integration.
...
def sceneflow_deep_prior(
    ...
    # If there is GT to compute metrics
    if flow:
...
def fit_sequence_of_scene_flow_field(
    ...
    csv_writer = csv.DictWriter(csv_file, metric_labels + ['traj_rmse'])
    ...
    for i in range(n_lidar_sweeps-1):
        ...
        flow_pred, info_dict_forward = sceneflow_deep_prior(pc1, pc2, None, info_dict, options)
    ...
    # SECTION: 2. Compute forward integration using the dense flow field

    # Initiate the neural prior
    deep_prior = Neural_Prior(filter_size=options.hidden_units).to(options.device)

    logger.info(f"* point cloud 0")
    P_0 = pc_list[0]
    traj = np.zeros([P_0.shape[0], n_lidar_sweeps, 3])
    d_euler_prev = 0
    for m in range(0, n_lidar_sweeps-1):
        logger.info(f"* next point cloud {m}")
        P_0_displaced = P_0 + d_euler_prev

        # ANCHOR: Interpolate closest flows (flow field interpolation)
        P_0_displaced_torch = torch.from_numpy(P_0_displaced).to(options.device).contiguous().unsqueeze(0).float()

        deep_prior.load_state_dict(info_dict_forward_lst[m]["state_dict_forward"])
        flow_forward_interp = deep_prior(P_0_displaced_torch).detach().cpu().numpy().squeeze(0)

        d_euler_prev = d_euler_prev + flow_forward_interp
        traj[:, m+1, :] = d_euler_prev
    
    rmse_traj = traj_metrics(
        torch.from_numpy(traj),
        torch.from_numpy(traj_gt),
        torch.from_numpy(traj_val_mask))
    logging.info(f" traj rmse: {rmse_traj:.3f}")

    cur_metrics = {label:round(cur_metrics[label].mean(), 4) for label in cur_metrics.keys()}
    cur_metrics['traj_rmse'] = rmse_traj
    csv_writer.writerow(cur_metrics)
    logging.info(cur_metrics)

    np.save(f"{exp_dir}/traj.npy", traj)
    return cur_metrics


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Long-term trajectory estimation with MBNSF and forward-Euler integration.")
    
    # ANCHOR: from config.py
    parser.add_argument('--exp_name', type=str, default='fit_MBNSF_FE_tl25_lr3_ep100', metavar='N', help='Name of the experiment.')
    ...
    parser.add_argument('--backward_flow', action='store_true', default=False, help='use backward flow or not.')
    ...
    metric_labels = ['epe', 'acc_strict', 'acc_relax', 'angle_error', 'outlier', 'traj_rmse', 'time']
    seq_metrics = {}
    for label in metric_labels:
    ...
    for fi_id in range(len(argoverse_tracking_val_log_ids)):
        ...
        seq_metrics['traj_rmse'][fi_id] = metrics['traj_rmse']
